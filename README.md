...
### How can I tell which computing resources are available on the Fornax Science Console?
  ...
  * In JupyterHub, open a terminal window, and use one of the below commands:
    ...
    * `top` gives info on both CPU and RAM usage. Some numerical packages (e.g. numpy) use multithreading, so you may see the CPU usage is more 100%. That means more than once CPU is used. You can see the individual CPU usage by pressing 1 while the `top` command is running.
