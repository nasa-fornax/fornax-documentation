...
2. If session contains running jobs with CPU activity up to the hard limit in 1.
3. A session is stopped after 15 minutes of no activity.
4. The user can override the 15 min limit by using the Keep-Alive functions under the Foranx menu. 
    ...
    - Stop a session: end a previously selected session by `Fornax -> Keep-alive -> Start Keep-alive Session`.
